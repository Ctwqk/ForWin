...
class SubWorldControlTests(unittest.TestCase):
    ...
    def test_subworld_admission_ignores_malformed_parenthetical_metadata_names(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        verdict = checker.check(
            ...
            WriterOutput(
                chapter_number=39,
                title="第39章",
                body="若槐宗邦的录音触发旧地铁封锁，馆员继续追查父亲留下的线索。" * 80,
                end_of_chapter_summary="馆员听到若槐宗邦的录音并确认封锁线索。",
                entity_mentions=[
                    EntityMention(entity_name="未现身）", entity_kind="character", is_named=True),
                    EntityMention(entity_name="若槐宗邦（远程触发封锁", entity_kind="character", is_named=True),
    ...
    def test_subworld_admission_normalizes_known_character_afterimage_annotation(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        verdict = checker.check(
            ...
            WriterOutput(
                chapter_number=41,
                title="第41章",
                body="猎迹者X的残留影像在旧地铁墙面浮现，灰鸦的残留影像也短暂闪回。" * 80,
                end_of_chapter_summary="残留影像确认旧地铁仍在广播。",
                entity_mentions=[
                    EntityMention(entity_name="猎迹者X（残留影像）", entity_kind="character", is_named=True),
                    EntityMention(entity_name="灰鸦（残留影像）", entity_kind="character", is_named=True),
                ],
            ),
        )
    ...
    def test_subworld_admission_normalizes_known_character_projection_annotation(self) -> None:
        ...
        self.assertEqual(unknown, ["灰鸦"])
        self.assertEqual(ContinuityChecker._candidate_character_name("蘅照夜（镜像投影）"), "蘅照夜")

    def test_subworld_admission_normalizes_known_character_remote_signal_intervention_annotation(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        self.assertEqual(unknown, ["灰鸦"])
        self.assertEqual(ContinuityChecker._candidate_character_name("蘅照夜（远程信号介入）"), "蘅照夜")
        self.assertEqual(ContinuityChecker._candidate_character_name("苏敏（林若溪）"), "苏敏（林若溪）")

    def test_subworld_admission_normalizes_known_character_recording_annotation(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        self.assertEqual(unknown, ["灰鸦"])
        self.assertEqual(ContinuityChecker._candidate_character_name("沈槐澜（录音）"), "沈槐澜")

    def test_subworld_admission_normalizes_known_character_duplicate_annotation(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        self.assertEqual(unknown, ["灰鸦"])
        self.assertEqual(ContinuityChecker._candidate_character_name("陈牧野（副本）"), "陈牧野")

    def test_subworld_admission_allows_known_parenthetical_identity_alias(self) -> None:
        class FakeRepo:
            def get_active_entities(self, _project_id: str) -> list[object]:
                return []
        ...
        verdict = checker.check(
            ...
            WriterOutput(
                chapter_number=97,
                title="第97章",
                body="沈槐澜也就是陆怀声在观测站完成身份揭示，灰鸦仍未获准进入本章。" * 80,
                end_of_chapter_summary="沈槐澜的陆怀声身份被揭示。",
                entity_mentions=[
                    EntityMention(entity_name="沈槐澜（陆怀声）", entity_kind="character", is_named=True),
                    EntityMention(entity_name="灰鸦", entity_kind="character", is_named=True),
                ],
            ),
        )
        ...
        self.assertEqual(unknown, ["灰鸦"])
        self.assertEqual(
            ContinuityChecker._candidate_character_name("沈槐澜（陆怀声）"),
            "沈槐澜（陆怀声）",
        )
    ...
    def test_subworld_admission_ignores_corpse_and_remains_references(self) -> None:
        ...
        verdict = checker.check(
            ...
            WriterOutput(
                ...
                body=(
                    "许澄跟随老锚进入冷却层密室。"
                    "尸体在平台上，馆员尸体的瞳孔倒映坐标，冷藏躯体旁留有旧港遇难者记录。"
                    "灰鸦仍未获准进入本章。"
                )
                * 30,
                end_of_chapter_summary="许澄与老锚提取坐标。",
                entity_mentions=[
                    EntityMention(entity_name="许澄", entity_kind="character", is_named=True),
                    EntityMention(entity_name="老锚", entity_kind="character", is_named=True),
                    EntityMention(entity_name="尸体", entity_kind="character", is_named=True),
                    EntityMention(entity_name="馆员尸体", entity_kind="character", is_named=True),
                    EntityMention(entity_name="冷藏躯体", entity_kind="character", is_named=True),
                    EntityMention(entity_name="旧港遇难者", entity_kind="character", is_named=True),
                    EntityMention(entity_name="灰鸦", entity_kind="character", is_named=True),
                ],
            ),
        )
    ...
    def test_current_plan_obligation_name_is_not_subworld_allowed_without_entry_target(self) -> None:
        ...
        try:
            ...
            repo = StateRepository(session)
            allowed_names = repo.get_allowed_entity_names(project.id, 18)
            verdict = ContinuityChecker(repo).check(
                project.id,
                WriterOutput(
                    chapter_number=18,
            ...
            try:
                ...
                verdict = ContinuityChecker(repo).check(
                    ...
                    WriterOutput(
                        chapter_number=8,
                        title="第八章",
                        body="灰鹞在沉箱酒吧压低声音交出陈牧野的锚点路径，陆明迅速记录。" * 80,
                        end_of_chapter_summary="灰鹞把陈牧野的旧委托交给陆明。",
                        entity_mentions=[
    ...
    def test_recent_summary_network_alias_without_handler_is_not_subworld_allowed_without_registration(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                project = updater.create_project(title="书", premise="p", genre="g")
                arc = updater.create_arc_plan(project.id, "弧线")
                chapter8 = updater.create_chapter_plan(
                    project_id=project.id,
                    arc_plan_id=arc.id,
                    chapter_number=8,
                    title="第八章",
                ...
                chapter8.status = "accepted"
                session.add(
                    ChapterDraft(
                        chapter_plan_id=chapter8.id,
                        version=1,
                        body_text="灰鹞网络遭X从内部击穿，陆明意识到交易窗口正在收窄。" * 40,
                        summary="灰鹞网络遭X从内部击穿，优先级升级为活体捕获。",
                        char_count=1200,
                    )
                )
                updater.create_chapter_plan(
                ...
                repo = StateRepository(session)
                allowed_names = repo.get_allowed_entity_names(project.id, 10)
                verdict = ContinuityChecker(repo).check(
                    project.id,
                    WriterOutput(
                        chapter_number=10,
                        title="第十章",
                        body="灰鹞在沉箱酒吧压低声音交出第三枚锚点路径，陆明迅速记录。" * 80,
                        end_of_chapter_summary="灰鹞提出条件性交易。",
                        entity_mentions=[
                            EntityMention(entity_name="灰鹞", entity_kind="character", is_named=True),
                            EntityMention(entity_name="陆明", entity_kind="character", is_named=True),
                        ],
    ...
    def test_roster_materialization_prefers_canonical_metadata_over_entity_lookup(self) -> None:
        ...
        self.assertEqual(result.character_id, "char_canonical_zhou")
        self.assertEqual(result.character_name, "周怀瑾")
        self.assertEqual([node.id for node in nodes if node.node_type == "character"], ["char_canonical_zhou"])
        self.assertIsNone(roster.entity_id)
        self.assertEqual(roster.display_name, "周怀瑾")
    ...
    def test_subworld_core_named_character_roster_dedupes_by_canonical_metadata(self) -> None:
        engine = get_engine(postgres_test_url("subworld-core-roster-canonical-dedupe"))
        init_db(engine)
        session = get_session_factory(engine)()
        try:
            updater = StateUpdater(session)
            ...
            nodes = [
                node
                for node in BookStateRepository(session).list_world_nodes(project.id)
                if node.node_type == "character" and node.name == "沈临川"
            ]
        ...
        self.assertEqual(roster_metadata[0].get("character_id"), nodes[0].id)
