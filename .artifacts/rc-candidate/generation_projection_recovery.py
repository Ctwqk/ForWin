...
def resolve_run_config(
    ...
    *,
    ...
    try:
        ...
        raise RunnerError(
            f"candidate manifest is invalid JSON: {candidate_manifest}"
        ) from exc
...
class EvidenceWriter:
    def __init__(
        ...
        *,
        ...
        self.evaluator = evaluator or load_module(
            "generation_projection_recovery_evaluator",
            EVALUATOR_PATH,
        )
        if report_validator is None:
            finalizer = load_module(
                "generation_projection_recovery_finalizer",
                FINALIZER_PATH,
            )
    ...
    def write_pass_report(
        ...
        *,
        ...
        if self.evaluator.stable_hash(reopened_assertions) != self.evaluator.stable_hash(
            assertions
        ):
        ...
        for name, payload in sorted((supplemental_artifacts or {}).items()):
            ...
            if (
                ...
                raise RunnerError(
                    f"invalid supplemental evidence name: {name}"
                )
        ...
        if self.evaluator.stable_hash(reopened_report) != self.evaluator.stable_hash(
            report
        ):
    ...
    def write_setup_blocked(
        ...
        *,
        ...
        if sha256_file(path) != report_hash:
            raise RunnerError(
                "setup_blocked report changed while reopening"
            )
        if reopened != payload:
    ...
    def _load_events(path: Path) -> list[dict[str, Any]]:
        finalizer = load_module(
            f"generation_projection_event_reader_{secrets.token_hex(4)}",
            FINALIZER_PATH,
        )
        try:
            return finalizer.load_verified_events(path)
    ...
    def _fault_times(
        ...
        action, time_field = FAULT_EVENT_BY_KIND[fault_kind]
        service = SERVICE_BY_FAULT[fault_kind]
        faults = [
            event
            for event in events
        ...
        recoveries = [
            ...
            for event in events
            if event.get("fault_id") == fault_id
            and event.get("action") == RECOVERY_EVENT_ACTION
            and event.get("service") == service
        ]
        if len(faults) != 1 or len(recoveries) != 1:
            raise RunnerError("controller event log has no unique fault/recovery pair")
        return (
            _required_text(faults[0].get(time_field), time_field),
            _required_text(recoveries[0].get("recovery_time"), "recovery_time"),
        )
