...
def build_character_personality_metrics(
    ...
    for node in nodes:
        ...
        if not isinstance(loadout_raw, dict) or not loadout_raw:
            ...
            try:
                ...
                if loadout.dominant is not None:
                    dominant_skills[loadout.dominant.skill] = (
                        dominant_skills.get(loadout.dominant.skill, 0) + 1
                    )
            except Exception:
                missing_loadout += 1
        if isinstance(assignment, dict):
            if assignment.get("manual_override"):
                manual_override += 1
            if (
                assignment.get("assignment_mode") == "fallback_minimal"
                or assignment.get("status") == "fallback_used"
            ):
                fallback_used += 1
    ...
    return {
        ...
        "character_creation_total": event_counts.get(
            DecisionEventType.CHARACTER_CREATED, 0
        ),
        "character_creation_auto_personality_assigned_total": event_counts.get(
            DecisionEventType.PERSONALITY_LOADOUT_AUTO_ASSIGNED, 0
        ),
        ...
        "most_used_dominant_skills": [
            ...
            for skill, count in sorted(
                dominant_skills.items(), key=lambda item: (-item[1], item[0])
            )
        ],
