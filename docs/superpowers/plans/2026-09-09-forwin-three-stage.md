...
### Task 22: Align the whole Writer context budget with its rendered input

The retained chapter-3 events report 44,419 estimated characters before trimming and 38,848 after, with two memories removed and summaries reduced from two to one. The original full ContextPack is unavailable, so these counts do not identify its dominant component or prove the cause of the BODY failure. Independent typed probes demonstrate that unrendered BookState map copies, entity state, map metadata and other residual fields still inflate the JSON estimate while all five Writer prompts remain identical. Repeated field exclusions leave two inconsistent definitions of Writer input.

**Bounded design:** the existing Writer prompt builder owns one context renderer with its existing single, preview, breakdown and scene layouts (stitch uses the scene layout). All five builders consume that renderer. Retrieval's soft character budget measures the largest of those rendered context blocks, on an isolated copy of mutable prompt telemetry. It excludes system/output instructions, schemas, skills and provisional scene drafts, whose existing prompt telemetry and bounds remain separate. Preserve exact prompt text, including the existing single-mode quality repetition; this task changes accounting, not prompt ablation. Replace the JSON/component estimator and its subtraction shortcuts, recalculating the rendered cost after each removal. Preserve configured limits, eviction order, minimum retained evidence, repair/reveal constraints, full source data and model-call count. The old empty-world JSON allowance is deliberately superseded by rendered-input accounting; required context may still exceed the soft budget. Do not alter relevance selection, add another memory store or infer sample quality from these tests.
