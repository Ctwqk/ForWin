...
- [x] 测试中段错误、标题相同但语义保留破坏、真实/伪造引用、无证据反对、timeout、超过8条条件、超预算JSON、API/UI unknown。
- [x] 先运行新回归，确认旧版失败；例如修复正文为头1500字+中段秘密泄露+尾1500字，断言引用可定位中段且不会被规则标题pass吞掉。
- [x] 实现完整输入/逐合同结果/证据核对/有界复核；保留持续error、硬review及Canonical边界。
- [x] 运行修复验证、repair service、eligibility和review API消费者测试，Ruff，独立复核后单独提交。
...
- [x] 故障注入存储异常，断言业务结果已提交、业务调用一次、trace outbox pending；新worker补传后同内容/key/hash且业务次数不增加。
- [x] 测试实际业务失败仍阻断、stale lease不提交、order_controls重跑的乱序trace不覆盖、上传成功但确认丢失可幂等重试。
- [x] 观察旧耦合失败，再实现同事务入outbox及只上传handler；不改outbox通用状态机。
- [x] 更新MinIO恢复断言，保留真实identity/fencing/恢复检查；运行相关默认与RC测试、Ruff，独立复核后单独提交。
...
- [x] 同日第一次active/waiting_review阻断，解除后第二次恰好入队，第三次及重启后不重复；先验证旧版失败。
- [x] 测试并发两个tick、提交失败回滚、入队成功后重启、发布重复释放和每日额度不超发。
- [x] 实现事务预留和重新评估，保持受支持的应用入口及现有策略，不靠进程内锁证明幂等。
- [x] 运行production/generation/outbox边界测试，Ruff，独立复核后单独提交。

## Task 4：整合、复核与交付

- [x] 独立审查三项交互及最终diff，修复可验证问题；另补持久化null丢失与并发设置覆盖日槽两项回归。
- [x] 最终默认完整suite、独立RC suite、Ruff和compileall；保存准确SHA和原始日志。
- [x] 更新CURRENT_DESIGN和本轮验证记录，提交；快进master并push后核对远端SHA。

最终验证候选：530824e；默认2218 passed / 1 skipped，独立RC1156 passed，Ruff/compileall通过。详细证据、初次失败和范围限制见[验证记录](../../operations/v5-autonomy-fixes-2026-09-04.md)。远端推送身份以本机integration.json及服务器核对结果为准。
