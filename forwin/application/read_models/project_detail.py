...
def build_project_detail(
    *,
    ...
    genesis_revision = _load_latest_genesis_revision_by_project(
        session, [project_id]
    ).get(project_id)
    book_state = BookStateQuery(session)
    as_of_chapter = book_state.repository.latest_available_chapter(project_id)
    ...
    chapter_infos = [
        ChapterInfo(
            ...
            title=plan.title,
            status=plan.status,
            char_count=draft_map.get(plan.id).char_count
            if draft_map.get(plan.id)
            else 0,
            summary=draft_map.get(plan.id).summary if draft_map.get(plan.id) else "",
            has_draft=draft_map.get(plan.id) is not None,
            has_review=bool(
                draft_map.get(plan.id) and draft_map.get(plan.id).id in review_draft_ids
            ),
            ...
            latest_repair_scope=normalize_repair_scope(
                getattr(latest_attempt_map.get(plan.chapter_number), "repair_scope", "")
                or "",
                default="",
            ),
        )
    ...
    generated_chapter_count = sum(
        1 for plan in plans if draft_map.get(plan.id) is not None
    )
    accepted_chapter_count = sum(1 for plan in plans if plan.status == "accepted")
    needs_review_chapter_count = sum(
        1 for plan in plans if plan.status == "needs_review"
    )

    runtime_maps = load_project_runtime_maps(session, [project_id])
    latest_checkpoint = _latest_band_checkpoint_by_project(session, [project_id]).get(
        project_id
    )
    policy_record = ProjectPolicyStore(session).load(project)
    decision_timeline = _decision_timeline_by_project(
        session, [project_id], limit=30
    ).get(project_id, [])
    narrative_constraints = _narrative_constraints_by_project(
