...
def _should_degrade_provisional_preview(exc: Exception) -> bool:
    ...
    if any(
        ...
        return True
    from .quality_gates import _is_transient_llm_like
...
class FinalizationStage:
    ...
    def _run_provisional_band_preview(
        ...
        *,
        ...
        for chapter_plan in chapter_plans:
            ...
            if summaries:
                previous = (list(context.previous_chapter_summaries) + summaries[-2:])[
                    -3:
                ]
    ...
    @staticmethod
    def _normalize_provisional_verdict(
        writer_output: WriterOutput, verdict: ReviewVerdict
    ) -> ReviewVerdict:
        return _normalize_provisional_verdict(writer_output, verdict)

    @staticmethod
    def _should_degrade_provisional_preview(exc: Exception) -> bool:
        return _should_degrade_provisional_preview(exc)
    ...
    def _build_provisional_fallback(
        *,
        ...
        return _build_provisional_fallback(
            chapter_plan=chapter_plan,
            current_time_label=current_time_label,
