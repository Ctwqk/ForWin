...
- P1-1 版本/冻结（095711a）、P1-3 容量（abb1690）和旧基线向前桥接（8371557）已分别提交，并完成独立评审及相关回归；临时评论隔离已通过独立评审与59项相关回归。P1-2 完整后缀重验正在实现，阶段长跑和 Stage 2/3 尚未验收。
...
- 最新隔离副本成功升级至 `0003_serial_capacity`：23条旧上传保留为 `uncertain`，23条独立 `reserved` 保护，15处原上传标题与候选标题的差异保留；18条 binding 仅通过原结果保存的直接 ID 恢复章号。没有生成发布回执或已公开保护。
...
当前接纳通过稳定 ChapterPlan 身份与 active commit 指针选择；历史接纳、增量和快照保留。修订请求保留原接纳状态，完整后缀核验尚未完成时拒绝历史切换。
...
11个热修复目录的来源提交均已在基线历史中。位置元数据回退、快照顺序及历史锚点的当前55项回归通过；旧重审入口由完整后缀验证取代，不能恢复仅凭旧增量重放的接纳。完整候选镜像验证和历史事务覆盖仍在进行，目录尚未删除。
