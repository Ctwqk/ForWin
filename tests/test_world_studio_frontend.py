...
class WorldStudioFrontendTests(unittest.TestCase):
    def test_world_studio_has_return_navigation_to_console(self) -> None:
        ...
        topbar_source = (REPO_ROOT / "frontend/world-studio/src/topbar-runtime.ts").read_text(encoding="utf-8")

        self.assertIn('<forwin-topbar active="world"></forwin-topbar>', index_source)
        self.assertIn('import "./topbar-runtime";', main_source)
        self.assertIn('aria-label", "ForWin primary navigation"', topbar_source)
        self.assertIn('href: "/"', topbar_source)
        self.assertIn('href: "/#task"', topbar_source)
        self.assertIn('href: "/world-studio"', topbar_source)
        self.assertIn('href: "/publishers"', topbar_source)
        self.assertIn('href: "/#config"', topbar_source)
        self.assertIn("customElements.define", topbar_source)
        self.assertIn("forwin-tab-change", topbar_source)
        self.assertIn("lang.toggle", topbar_source)
        ...
        self.assertIn("Books", topbar_source)
        self.assertIn("Tasks", topbar_source)
        self.assertIn("Archive", topbar_source)
        self.assertIn("Publish", topbar_source)
        self.assertIn("Settings", topbar_source)
        self.assertIn(".nav-tabs", css_source)
        self.assertIn(".nav-tabs--secondary", css_source)
        self.assertIn(".topbar-shell", css_source)
        self.assertIn(".lang-toggle", css_source)
        self.assertIn(".forwin-topbar-wrap", css_source)
