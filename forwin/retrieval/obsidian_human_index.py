...
class ObsidianHumanVectorIndex:
    ...
    def rebuild_project(self, project_id: str, *, vault_root: Path) -> dict[str, Any]:
        ...
        self.client.delete(
            collection_name=self.collection_name,
            points_selector=self._rest.FilterSelector(filter=self._project_filter(project_id)),
            wait=True,
        )
        embeddings = self.embedder.embed([section["text"] for section in sections]) if sections else []
        points = [
            self._rest.PointStruct(
                id=_point_id(project_id, section["vault_path"], section["section_name"]),
                vector=embedding,
                payload=section,
            )
