...
test('background throttles login QR notifications at the backend boundary', async () => {
  ...
  assert.match(source, /settings\.loginQrNotificationsAllowed\s*!==\s*true/);
...
test('background login QR extraction asks matching child frames before screenshot fallback', async () => {
  ...
  assert.match(source, /extractLoginQrWithScripting/);
  assert.match(source, /extensionApi\.scripting/);
...
test('background opens a dashboard probe tab for cookie-backed heartbeat verification', async () => {
  ...
  assert.match(source, /hasVerificationTab/);
