...
def _shutdown_runtime_state() -> None:
    ...
    if api_state._pipeline is not None:
        try:
            ...
            logger.debug("Ignoring pipeline LLM client shutdown error.", exc_info=True)
        try:
...
async def lifespan(app: FastAPI):
    ...
    if api_state._pipeline is None:
        ...
        api_state._pipeline = api_state._runtime_container.build_chapter_pipeline()
        with api_state._SessionFactory() as bootstrap_session:
            created_envelopes = (
                api_state._pipeline.arc_envelope_manager.backfill_missing_resolutions(
...
_registered_route_handlers = api_route_registry.register_api_routes(
    ...
    deps=api_route_registry.ApiRouteDeps(
        ...
        project=api_route_registry.ProjectDeps(
            ...
            active_genesis_revision=lambda session, project: _active_genesis_revision(
                session, project
            ),
        ...
        project_control=api_route_registry.ProjectControlDeps(
            ...
            validate_constraint_payload=lambda **kwargs: _validate_constraint_payload(
                **kwargs
            ),
            ...
            build_audit_insights=lambda session, *, project_id: _build_audit_insights(
